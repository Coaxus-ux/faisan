...
export default function SearchComponent() {
    ...
    const {getAllFertilisation, getFertilisation} = useFertilisationStore();
    ...
    return (
        <section className="shadow bg-white rounded p-5 flex flex-col gap-3">
            <div className="flex justify-between">
                <div className="flex gap-2 items-center">
                    <TbReportSearch size="30"/>
                    <h3 className="font-bold text-2xl">Buscador de registros.</h3>
                </div>
                <Button color="success" variant="flat" className="px-8" onClick={getAllFertilisation}>Crear nuevo</Button>
            </div>
            <div className="flex gap-2 items-center h-16">
                <div className="flex gap-1 items-center">
                    ...
                    <Autocomplete
                        ...
                        selectedKey={filters.birthType.value}
                    >
                        {(item) => <AutocompleteItem key={item.value}>{item.label}</AutocompleteItem>}
                    </Autocomplete>
