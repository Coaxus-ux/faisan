...
export default function AnimalDetailsCard({ animal }) {
  ...
  const months =
    differenceInCalendarMonths(dateNow, new Date(animal.animalBirthDate)) % 12;
  let days =
    (differenceInCalendarDays(dateNow, new Date(animal.animalBirthDate)) % 30) -
    1;
  days < 0 ? (days = 0) : days;
  const years = differenceInCalendarYears(
    dateNow,
    new Date(animal.animalBirthDate),
  );
  return (
    <Card className="p-5 h-[508px]">
      <CardHeader>
      ...
      <CardBody className="grid gap-4">
        ...
        <div className="grid sm:grid-cols-2 gap-2">
          ...
          <div>
            ...
            <p className="italic">
              {animal.fertilisationType.typeFertilisation}
            </p>
          </div>
        </div>
        <div className="grid sm:grid-cols-2 gap-2">
          <div>
            ...
            <div
              ...
              style={{ backgroundColor: animal.animalColor.animalHex }}
            />
          </div>
        </div>
      </CardBody>
